...
- HTTP slice tests must exercise the real `AdminHttpServer` through an
  ephemeral port, following `AdminHttpSliceTestBase`.
- Assert the JSON wire format and HTTP status codes consumed by the admin UI.
- Use `json-unit`'s AssertJ integration (`assertThatJson`) for structured JSON
  response assertions instead of asserting individual JSON fragments with
  `contains`.
- Use `IGNORING_ARRAY_ORDER` when collection ordering is not part of the
  behavior under test; do not ignore extra fields unless the test explicitly
  permits them.
...
- Keep handler-only tests for pure handler behavior; do not replace slice
  coverage when testing routing, serialization, server bootstrap, or CORS.
- Use `Req` and `Resp` as suffixes for paired request/response variables and
  fixtures and use java `var`:
...
- For a simple one-shot request where constructing a request object adds no
  value, a descriptive response variable such as `topicsResp` is sufficient.
...
- Do not introduce a new abstraction when an existing helper or base class
